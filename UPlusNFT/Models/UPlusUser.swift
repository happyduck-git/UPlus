...
struct UPlusUser: Codable {
     ...
     let userRewards: [String]? // 사용자가 획득한 보상 아이템 다큐먼트를 배열로 갖는다.
     let userNfts: [String]? //사용자가 획득한 NFT 다큐먼트를 배열로 갖는다.
     let userMissions: [String]? //사용자가 참여한 미션 다큐먼트를 배열로 갖는다.
     let userPointHistory: [PointHistory]?
...
struct PointHistory: Codable {
    ...
    let userPointTime: Timestamp //사용자가 포인트를 획득한 날짜를 가리킨다. 날짜와는 다르게 시각은 항상 00시 00분을 가리킨다.
